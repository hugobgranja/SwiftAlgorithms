...
class BellmanFordSPTests: XCTestCase {
    ...
    func testHasPath() {
        let sp = BellmanFordSP(graph: graph, source: 0)
    ...
    func testDistance() {
        ...
        XCTAssertEqual(sp.distance(to: 2), 14)
        XCTAssertEqual(sp.distance(to: 3), 17)
        XCTAssertEqual(sp.distance(to: 4), 9)
        XCTAssertEqual(sp.distance(to: 5), 13)
        XCTAssertEqual(sp.distance(to: 6), 25)
    ...
    func testPathToThree() {
        ...
        let expected = [
            ...
            DirectedEdge(from: 4, to: 5, weight: 4),
    ...
    func testNegativeCycle() {
        graph.addEdge(from: 4, to: 7, weight: 1)
        graph.addEdge(from: 5, to: 4, weight: -4)
        graph.addEdge(from: 7, to: 5, weight: 2)
        let sp = BellmanFordSP(graph: graph, source: 0)
        
        XCTAssertTrue(sp.hasNegativeCycle())
        XCTAssertEqual(sp.getNegativeCycle(), [4,7,5,4])
    }
    
}

extension BellmanFordSPTests {
